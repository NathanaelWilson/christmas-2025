...
const images = {
  giftInfo: "/Gift.png",
  paperTexture: "/Postcard.png",
  dresscodeInfo: "/Dresscode.png",
}
...
export default function DigitalInvitation({ data, onClose }: DigitalInvitationProps) {
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      const filename = `Invitation-${data?.fullName?.replace(/\s+/g, '-') || "Guest"}.png`
        ...
        if (navigator.canShare({ files: [file] })) {
          try {
            ...
            if (shareError.name !== 'AbortError') {
               console.warn("Share API gagal, fallback ke download.", shareError)
            } else {
               setIsGenerating(false)
               return
            }
        ...
        >
            ...
            <div className="relative w-full aspect-[3/2] rounded-lg overflow-hidden shadow-sm border border-stone-200 bg-white">
              ...
              >
                ...
                <div className="h-full w-full p-5 flex flex-col justify-between relative" style={{ backgroundColor: "rgba(255,255,255,0.7)" }}>
                    
                    {/* Header: To & Date */}
                    <div className="flex justify-between items-start">
                        <div className="font-serif italic border-b border-rose-900/30 text-stone-800 pb-1">
                            <span className="text-xs not-italic mr-1 font-sans text-stone-500">To:</span>
                            <span className="text-red-800 font-bold text-lg">{data?.fullName || "Guest"}</span>
                        </div>
                        ...
                        <div className="w-12 h-12 border-2 border-red-800/20 rounded-full flex items-center justify-center -rotate-12 transform translate-x-1 -translate-y-1">
                           <span className="font-bold text-[7px] text-center uppercase font-mono text-red-800/30">Air Mail<br/>2025</span>
                        </div>
                    </div>
                    ...
                    <div className="text-center space-y-1 my-1">
                        ...
                        <p className="text-stone-600 italic text-xs">"I'll be Home for Christmas"</p>
                    ...
                    <div className="flex justify-between items-center bg-stone-100/60 border border-stone-200/50 rounded-lg p-2 text-[10px] sm:text-xs font-mono text-stone-700">
                        <div className="flex items-center gap-1"><span>📅</span><span className="font-semibold">{eventDetails.date}</span></div>
                        <div className="w-px h-3 bg-stone-300 mx-1"></div>
                        <div className="flex items-center gap-1"><span>🕖</span><span>{eventDetails.time}</span></div>
                        <div className="w-px h-3 bg-stone-300 mx-1"></div>
                        <div className="flex items-center gap-1"><span>📍</span><span>{eventDetails.location}</span></div>
                    </div>
            ...
            <div className="text-center">
                <p className="font-mono text-[9px] text-stone-400 tracking-widest uppercase">GMS Christmas 2025</p>
            </div>
